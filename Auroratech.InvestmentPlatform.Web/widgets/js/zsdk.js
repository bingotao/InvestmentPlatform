(function () {
    ...
    var $pqDropDownSFYBP = $app.find('.dropdown.sfybp');
    var $pqDownSFYBP = $pqDropDownSFYBP.find('ul');
    for (var i = 0; i < sfybp.length ; i++) {
        var bflx = sfybp[i];
        var $li = $('<li>' + bflx.mc + '</li>');
        $li.data('sfybp', bflx);
        $li.appendTo($pqDownSFYBP);
    }
    $pqDropDownSFYBP.find('li').click(function () {
        var $this = $(this);
        var bflx = $this.data('sfybp');
        var txt = bflx.mc;
        $this.parent().parent().find('.pq-text').html(txt).data('bflx', bflx).val(bflx.dm);
    });
    $pqDropDownSFYBP.find('li').first().click();
    $pqDropDownSFYBP.click(function () {
        var $this = $(this);
