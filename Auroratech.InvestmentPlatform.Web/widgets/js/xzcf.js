(function () {
    ...
    var $pqDropDownLX = $app.find('.dropdown.lx');
    var $pqDownLX = $pqDropDownLX.find('ul');
    for (var i = 0; i < lxs.length ; i++) {
        var lx = lxs[i];
        var $li = $('<li>' + lx.mc + '</li>');
        $li.data('lx', lx);
        $li.appendTo($pqDownLX);
    }
    $pqDropDownLX.find('li').click(function () {
        var $this = $(this);
        var lx = $this.data('lx');
        var txt = lx.mc;
        $this.parent().parent().find('.pq-text').html(txt).data('lx', lx).val(lx.dm);
    });
    $pqDropDownLX.find('li').first().click();
    $pqDropDownLX.click(function () {
        var $this = $(this);
