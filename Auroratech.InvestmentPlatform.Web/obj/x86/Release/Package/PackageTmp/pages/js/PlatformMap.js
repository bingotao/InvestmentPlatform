...
$(function () {
    ...
    function resize() {
        ...
        $mapContainer.width(document.documentElement.clientWidth - $slider.innerWidth());
...
function initMap() {
    giscom.initMap('mapMain', function (myMap) {
        ...
        //    var layer = map.getLayer('xzqh');
        //    if (layer && layer.loaded) {
        //        giscom.initOverView(layer);
        //    }
        //    //clearInterval(h);
        //}, 1000);
...
function mapMultiExport(mapType) {
    var layerUrl = 'http://127.0.0.1:6080/arcgis/rest/services/XQ/XQ_' + mapType + '/MapServer/0';
    require(['esri/tasks/query', 'esri/tasks/QueryTask', 'esri/tasks/PrintTemplate', 'esri/tasks/PrintParameters', 'esri/tasks/PrintTask'], function (Query, QueryTask, PrintTemplate, PrintParameters, PrintTask) {
        ...
        var printUrl = 'http://127.0.0.1:6080/arcgis/rest/services/Utilities/PrintingTools/GPServer/Export%20Web%20Map%20Task';
        var query = new Query();
            ...
            function printMap(callback) {
                if (i != fts.length) {
                    ...
                    var handle = map.on('extent-change', function () {
                        handle.remove();
                        var oId = ft.attributes['OBJECTID'];
